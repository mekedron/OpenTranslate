...
export const SITE_CONTROL_URL_WINDOW_KEY = "__OPENTRANSLATE_SITE_CONTROL_URL__" as const

declare global {
  interface Window {
    __OPENTRANSLATE_SITE_CONTROL_URL__?: string
  }
}
...
export function getEffectiveSiteControlUrl(url: string): string {
  ...
  return resolveEffectiveSiteControlUrl(url, window.__OPENTRANSLATE_SITE_CONTROL_URL__)
}
...
export function clearEffectiveSiteControlUrl(): void {
  ...
  delete window.__OPENTRANSLATE_SITE_CONTROL_URL__
}
