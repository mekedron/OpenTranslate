...
describe("spinner", () => {
  ...
  it("ensures preset styles on the document before appending the spinner", async () => {
    ...
    ensurePresetStylesMock.mockImplementation((root: Document | ShadowRoot) => {
      ...
      expect(wrapper.querySelector(".opentranslate-spinner")).toBeNull()

      const style = document.createElement("style")
      style.id = "opentranslate-preset-styles"
    ...
    expect(document.head.querySelector("#opentranslate-preset-styles")).not.toBeNull()
    expect(wrapper.lastElementChild).toBe(spinner)
    expect(spinner.className).toBe("opentranslate-spinner")
  }, 10_000)
  ...
  it("ensures preset styles on the containing shadow root before appending the spinner", async () => {
    ...
    expect(ensurePresetStylesMock).toHaveBeenCalledOnce()
    expect(shadow.querySelector("#opentranslate-preset-styles")).not.toBeNull()
    expect(wrapper.lastElementChild).toBe(spinner)
    expect(spinner.className).toBe("opentranslate-spinner")
  }, 10_000)
  ...
  it("uses a thin gray spinner arc without a background ring", async () => {
    ...
    expect(spinner.style.borderTopColor).toBe("var(--opentranslate-muted-foreground)")
