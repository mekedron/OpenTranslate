...
describe("getModelById", () => {
  ...
  it("passes Azure settings and resolves the deployment name with languageModel", async () => {
      ...
      providersConfig: [
        {
          ...
          model: {
            model: "gpt-5.4-mini",
            isCustomModel: true,
            customModel: "opentranslate-gpt-4o",
          },
          providerSpecificSettings: {
            apiMode: "responses",
            resourceName: "opentranslate-openai",
    ...
    expect(azureLanguageModelMock).toHaveBeenCalledWith("opentranslate-gpt-4o")
  ...
  it("uses Azure chat completions when API mode is chat", async () => {
    ...
    expect(createAzureMock.mock.calls[0]?.[0]).not.toHaveProperty("apiMode")
    expect(createAzureMock.mock.calls[0]?.[0]).not.toHaveProperty("region")
    expect(azureChatModelMock).toHaveBeenCalledWith("opentranslate-gpt-4o")
