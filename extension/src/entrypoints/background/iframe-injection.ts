...
export function setupIframeInjection() {
  // Safari/iOS may not expose scripting/webNavigation. Skip late iframe injection
  // gracefully — the top frame and manifest-declared content scripts still translate.
  if (
    typeof browser.scripting?.executeScript !== "function"
    || typeof browser.webNavigation?.onCompleted === "undefined"
  ) {
    logger.info("[Background][IframeInjection] scripting/webNavigation unavailable; skipping late iframe injection")
    return
  }
