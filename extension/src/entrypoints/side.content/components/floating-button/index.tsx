...
import openTranslateLogo from "@/assets/icons/opentranslate.png?url&no-inline"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const openTranslateLogoUrl = new URL(openTranslateLogo, browser.runtime.getURL("/")).href
...
export default function FloatingButton() {
  ...
  // Touch-first devices (iOS Safari) can't hover, so a tap opens the menu there.
  ...
  // Touch: collapse the menu back to its peek state when the next tap lands
  // outside the button (no `mouseleave` fires on touch). Skipped while the close
  // dropdown is open — base-ui dismisses that on its own outside tap.
  useEffect(() => {
    if (!isCoarsePointer || !isHitAreaExpanded || isDropdownOpen)
      return
    ...
    document.addEventListener("pointerdown", handlePointerDownAway)
    return () => document.removeEventListener("pointerdown", handlePointerDownAway)
  }, [isCoarsePointer, isHitAreaExpanded, isDropdownOpen])
  ...
  const finishPointerInteraction = (
    ...
    if (shouldTriggerClick) {
      // On touch the frog is the menu toggle (there's no hover to open/close it):
      // tap to open, tap again to collapse. Translation happens from the translate
      // button inside the menu. Desktop keeps tap = toggle translation.
      if (isCoarsePointer) {
        setIsHitAreaExpanded(expanded => !expanded)
        return
      }
  ...
  const handleMouseEnter = () => {
    // Touch uses tap-to-open (see finishPointerInteraction); ignore emulated hover.
    if (isCoarsePointer || isDraggingButton)
      return
    setIsHitAreaExpanded(true)
  }

  const handleMouseLeave = () => {
    if (isCoarsePointer || isDropdownOpen || isDraggingButton)
      return
    setIsHitAreaExpanded(false)
  }
    ...
    >
      ...
      <div className="relative">
        <div
          ...
          className={cn(
            // `touch-action: none` so iOS fires the pointer gesture instead of
            // treating the first touch as a page scroll (otherwise the tap and
            // drag-start never register — "clickable but not touchable").
            "border-border relative flex h-10 touch-none items-center bg-white shadow-lg transition-transform duration-300 dark:bg-neutral-900",
            isDraggingButton
              ? "w-10 justify-center rounded-full border cursor-grabbing opacity-100"
