...
export default function LanguageOptionsSelector() {
  ...
  // Show only the language NAME (e.g. "English") in the compact trigger — the full
  // label "English (English)" overflows the narrow trigger and truncates on iOS.
  // The source/target subtitle below already conveys the Auto/Target role.
  const sourceLangLabel = currentSourceItem?.name ?? getLanguageName(detectedCode)
