...
declare global {
  interface Window {
    __OPENTRANSLATE_SELECTION_INJECTED__?: boolean
  }
}
...
export default defineContentScript({
  ...
  async main(ctx) {
    ...
    if (window.__OPENTRANSLATE_SELECTION_INJECTED__)
      return
    window.__OPENTRANSLATE_SELECTION_INJECTED__ = true

    ctx.onInvalidated(() => {
      window.__OPENTRANSLATE_SELECTION_INJECTED__ = false
      clearEffectiveSiteControlUrl()
    })

    // Bridge the current selection text to the extension popup (iOS selection
    // translate). The popup has no access to the page DOM, so it asks the content
    // script. iOS may clear the selection when the popup opens, so we also remember
    // the last non-empty selection and fall back to it.
    let lastSelection = ""
    ...
    onMessage("getActiveSelectionText", () => {
      const liveSelection = window.getSelection()?.toString()?.trim()
      return liveSelection || lastSelection || ""
    })
